...
const current = 'tiktok-shop-creator-live-brand-system';
const previous = 'tiktok-shop-creator-merchant-content-growth-stage';
const outputDir = process.env.QA_OUTPUT || 'outputs/2026-09-14-training38-production-qa';
...
(async () => {
  ...
  const expected = [current, previous, 'tiktok-shop-merchant-content-compounding-system'];
  const indexPassed = reports.index.every((report) =>
    !report.overflow && report.brokenImages.length === 0 &&
    report.script?.includes('0180a54a47d5') &&
