...
const current = 'tiktok-shop-creator-merchant-content-growth-stage';
const previous = 'tiktok-shop-merchant-content-compounding-system';

async function pageReport(page, slug, lang, viewportName) {
  const suffix = lang === 'zh-CN' ? '?lang=zh' : '';
  await page.goto(`${base}/blog/${slug}${suffix}`, { waitUntil: 'networkidle' });
...
(async () => {
    ...
    for (const slug of [current, previous]) {
      for (const lang of ['en', 'zh-CN']) {
        ...
        await page.screenshot({
          path: `outputs/2026-09-14-training37-production-qa/${slug}-${lang}-${viewport.name}.png`,
          fullPage: false,
        });
      }
    }
  ...
  const expected = [current, previous, 'tiktok-shop-video-photo-live-content-job'];
  const indexPassed = reports.index.every((report) =>
    !report.overflow && report.brokenImages.length === 0 &&
    report.script?.includes('60257bc2d125') &&
