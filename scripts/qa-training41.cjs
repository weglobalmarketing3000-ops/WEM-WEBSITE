...
const current = 'tiktok-shop-product-readiness-content-creators-paid-scale';
const previous = 'tiktok-shop-listing-discovery-strategy';
const outputDir = process.env.QA_OUTPUT || 'outputs/2026-09-17-training41-production-qa';
...
(async () => {
  ...
  const expected = [current, previous, 'tiktok-shop-search-shop-tab-content-discovery'];
  const indexPassed = reports.index.every((report) =>
    !report.overflow && report.brokenImages.length === 0 &&
    report.script?.includes('0c9ef5d36955') &&
