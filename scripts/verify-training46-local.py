"""Read-only static verification of the target-only local training-46 package."""
import hashlib, html, json, re
from pathlib import Path

repo=Path(__file__).resolve().parents[1]
base=repo/'outputs/patches/2026-09-24-training46-consumer-electronics'
slug='tiktok-shop-consumer-electronics-operating-system'
date='2026-09-24'
...
checks['source_rank1_unique']=list_js.count('slug:"'+slug+'"')==1 and entries[:4]==[
    slug,'tiktok-shop-growth-autopilot-vs-gmv-max-pro','tiktok-shop-growth-autopilot-roi-protection','tiktok-shop-jewelry-category-operating-system'
]
checks['history_retained']='tiktok-shop-growth-autopilot-beta-guide' in entries
...
checks['source_links']=all(s['url'] in page for s in json.loads((repo/'content/training46-consumer-electronics.json').read_text())['sources'])
...
checks['all_images']=all((((base/a) if (base/a).exists() else (repo/'ui_kits/website'/a)).exists() and ((base/a) if (base/a).exists() else (repo/'ui_kits/website'/a)).stat().st_size>100) for a in assets)
...
out=repo/'outputs/2026-09-24-training46-local-verify.json'
