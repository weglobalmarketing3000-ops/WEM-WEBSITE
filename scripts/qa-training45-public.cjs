...
const patchDir = path.join(root, 'outputs/patches/2026-09-21-training45-scoped-local');
const out = path.join(root, 'outputs/2026-09-21-training45-public-qa');
...
const expected = [comparison, roi, current, foundation];
const expectedDates = ['Sep 23, 2026', 'Sep 22, 2026', 'Sep 21, 2026', 'Sep 21, 2026'];
...
(async () => {
  ...
  report.checks.discovery = sitemap.includes(`<loc>${articleUrl}</loc><lastmod>2026-09-21</lastmod>`) && sitemap.includes(`<loc>${articleUrl}?lang=zh</loc><lastmod>2026-09-21</lastmod>`) && llms.includes(articleUrl);
