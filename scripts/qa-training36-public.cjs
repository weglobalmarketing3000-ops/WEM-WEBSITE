...
const current = 'tiktok-shop-merchant-content-compounding-system';
const previous = 'tiktok-shop-video-photo-live-content-job';
...
(async () => {
    ...
    for (const slug of [current, previous]) {
      for (const lang of ['en', 'zh-CN']) {
        ...
        await page.screenshot({
          path: `outputs/2026-09-12-training36-production-qa/${slug}-${lang}-${viewport.name}.png`,
          fullPage: false,
        });
      }
    }
  ...
  const expected = [current, previous, 'tiktok-shop-short-video-localization-us-shoppers'];
  const indexPassed = reports.index.every((report) =>
    !report.overflow && report.brokenImages.length === 0 &&
    report.script?.includes('9521c0f67804') &&
