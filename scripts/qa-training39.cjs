...
const current = 'tiktok-shop-search-shop-tab-content-discovery';
const previous = 'tiktok-shop-creator-live-brand-system';
const outputDir = process.env.QA_OUTPUT || 'outputs/2026-09-17-training39-production-qa';
...
(async () => {
  ...
  const expected = [current, previous, 'tiktok-shop-creator-merchant-content-growth-stage'];
  const indexPassed = reports.index.every((report) =>
    !report.overflow && report.brokenImages.length === 0 &&
    report.script?.includes('40ab5f0d8bd1') &&
